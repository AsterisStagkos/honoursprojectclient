...
public class AppDetailActivity extends ActionBarActivity {
	...
	public void setInstall(boolean shouldInstall) {
		...
		if (downloadOrInstall) {			
			...
			// We use posts to touch views from outside the threads they were created in.
	...
	public void sendDownloadRequest(String assetID, String appName) {
		...
		//conn.sendData(connectionSocket, "download " + assetID + " " + appName + " " + whichExperiment + " " + GetUsernameTask.getDownloadToken() + " " + GetUsernameTask.getAndroidId());
		SearchableActivity.web.sendMessage(("download " + assetID + " " + appName + " " + whichExperiment + " " + GetUsernameTask.getDownloadToken() + " " + GetUsernameTask.getAndroidId()).getBytes());
	...
	private void awaitResponse(String option, final String appName) {
		if (option.equals("download")) {
			//setInstall(false);
			mProgress = new ProgressBar(AppDetailActivity.this, null, android.R.attr.progressBarStyleHorizontal);
		   ...
		   // setContentView(detailLayout);
		    ...
		    threadReceive.start();
		    //websockets.receive(appName);
		   // conn.receive(connectionSocket, getApplicationContext(), appName + ".apk", thisActivity);
	             ...
	             public void run() {
	              ...
	              //   mProgress.setProgress(mProgressStatus);
	                     ...
	                     mHandler.post(new Runnable() {
	                         public void run() {
	                        	// Log.d("mHandler", "progress set to: " + mProgressStatus);
	                             mProgress.setProgress(mProgressStatus);
	                         }
	                     });
	                 }
